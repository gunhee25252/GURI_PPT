...
async function pdfToPng(pdfFile, props) {
    ...
    const pdfFileBuffer = await getPdfFileBuffer(pdfFile);
    ...
    const returnMetadataOnly = props?.returnMetadataOnly ?? false;
    ...
    if (props?.outputFolder !== undefined && !returnMetadataOnly) {
        await node_fs_1.promises.mkdir((0, node_path_1.join)(process.cwd(), props.outputFolder), { recursive: true });
    }
    // Process each page
    const pngPagesOutput = [];
    try {
        const pageViewportScale = props?.viewportScale ?? const_1.PDF_TO_PNG_OPTIONS_DEFAULTS.viewportScale;
        const defaultMask = typeof pdfFile === 'string'
            ? (0, node_path_1.parse)(pdfFile).name
        ...
        const shouldReturnContent = returnMetadataOnly
            ? false
            : props?.outputFolder
                ? true
        ...
        if (props?.processPagesInParallel === true) {
            ...
            const concurrencyLimit = props?.concurrencyLimit ?? const_1.PDF_TO_PNG_OPTIONS_DEFAULTS.concurrencyLimit;
            for (let i = 0; i < validPagesToProcess.length; i += concurrencyLimit) {
                ...
                const batchResults = await Promise.all(batch.map(async (pageNumber) => {
                    const pageOutput = await processPdfPage(pdfDocument, props?.outputFileMaskFunc?.(pageNumber) ?? `${defaultMask}_page_${pageNumber}.png`, pageNumber, pageViewportScale, shouldReturnContent, returnMetadataOnly);
                    if (props?.outputFolder && !returnMetadataOnly) {
                        await savePNGfile(pageOutput, (0, node_path_1.join)(process.cwd(), props.outputFolder));
 ...
 * @async
 */
async function getPdfFileBuffer(pdfFile) {
    const isString = typeof pdfFile === 'string';
    const pdfFileBuffer = isString
        ? await (async () => {
 ...
 */
async function processPdfPage(pdf, pageName, pageNumber, pageViewportScale, returnPageContent, returnMetadataOnly) {
    ...
    const canvasFactory = pdf.canvasFactory ? pdf.canvasFactory : new node_canvas_factory_1.NodeCanvasFactory();
 ...
 */
async function savePNGfile(pngPageOutput, outputFolder) {
    pngPageOutput.path = (0, node_path_1.join)(outputFolder, pngPageOutput.name);
